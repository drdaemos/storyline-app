...
'I see...', a whisper responded from his side.`
      ...
      expect(output).toContain(
        `<span class="text-default">Georgie's apartment glowed under the dimmed lights. She was wearing a robe, clinging to her 5'10" frame, and walking towards Josh.</span>`
      )
      expect(output).toContain(
        `<span class="text-highlighted">"I invited you here for a reason..."</span>`
      )
      expect(output).toContain(`<span class="text-highlighted">'I see...'</span>`)
      expect(output).toContain(`, she confesses. *Her skin is trembling with hesitation.`)
      expect(output).toContain(`, a whisper responded from his side.`)
    })
  })
