...
class ConversationMemory:
    ...
    def add_message(self, character_id: str, session_id: str, role: str, content: str, message_type: str = 'conversation') -> int:
        ...
        with self.db_config.create_session() as session:
            ...
            max_offset = session.query(func.coalesce(func.max(Message.offset), -1)).filter(
    ...
    def get_session_messages(self, session_id: str, limit: int | None = None) -> list[GenericMessage]:
        ...
        """
        with self.db_config.create_session() as session:
            query = session.query(Message).filter(Message.session_id == session_id).order_by(Message.offset)
    ...
    def get_session_details(self, session_id: str) -> dict[str, Any] | None:
        ...
        with self.db_config.create_session() as session:
            result = session.query(
                ...
                Message.character_id, Message.session_id
            ).first()
    ...
    def get_recent_messages(self, session_id: str, limit: int = 10, type: str | None = None) -> list[GenericMessage]:
        ...
        with self.db_config.create_session() as session:
            ...
            subquery = session.query(Message).filter(Message.session_id == session_id)
    ...
    def clear_character_memory(self, character_id: str) -> int:
        ...
        """
        with self.db_config.create_session() as session:
            count = session.query(Message).filter(Message.character_id == character_id).delete()
            session.commit()
            return count
