...
class TestNPCAgent:
    def test_init(self):
        with patch('src.agent.LLM') as mock_llm, \
    ...
    def test_init_default_name(self):
        with patch('src.agent.LLM'), \
             patch('src.agent.Agent'):
            
            agent = NPCAgent(
                role="Merchant",
                backstory="A traveling trader"
            )
    ...
    def test_respond(self, mock_llm, mock_agent, mock_task, mock_crew):
        ...
        agent = NPCAgent(
            role="Innkeeper",
            backstory="Friendly innkeeper who knows everyone",
            name="Martha"
        )
        ...
        assert 'User message: Hello there!' in task_call_args["description"]
        ...
        mock_crew.assert_called_once_with(
            agents=[mock_agent.return_value],
            tasks=[mock_task.return_value],
            verbose=False
        )
    ...
    def test_respond_with_memory_context(self, mock_llm, mock_agent, mock_task, mock_crew):
        ...
        agent = NPCAgent(
            role="Guard",
            backstory="Village guard who remembers faces",
            name="Captain Rex"
        )
        ...
        assert "User message: Do you remember me?" in task_description
    ...
    def test_respond_includes_few_shot_examples(self, mock_llm, mock_agent, mock_task, mock_crew):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('src.agent.FewShotLoader') as mock_loader_class:
                ...
                assert "If few-shot examples are provided above, emulate their writing style" in task_description
                assert "You are Sir Lancelot" in task_description
                assert "User message: Hello!" in task_description
    ...
    def test_respond_without_few_shot_examples(self, mock_llm, mock_agent, mock_task, mock_crew):
        ...
        with patch('src.agent.FewShotLoader') as mock_loader_class:
            ...
            assert "User message: Hi!" in task_description
