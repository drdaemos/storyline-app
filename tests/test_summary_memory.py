...
class TestSummaryMemory:
    def setup_method(self):
        ...
        test_db_path = Path(self.temp_dir) / "test_summaries.db"
    ...
    def test_add_summary_valid(self):
        ...
        summary_id = self.memory.add_summary(self.character_id, self.session_id, "User introduced themselves and asked about the case.", 0, 2)
    ...
    def test_add_summary_invalid_offsets(self):
        ...
        with pytest.raises(ValueError, match="Offsets must be non-negative"):
            self.memory.add_summary(self.character_id, self.session_id, "Invalid summary", -1, 2)

        with pytest.raises(ValueError, match="Offsets must be non-negative"):
            self.memory.add_summary(self.character_id, self.session_id, "Invalid summary", 0, -1)

        # Test start_offset > end_offset
        with pytest.raises(ValueError, match="start_offset must be <= end_offset"):
            self.memory.add_summary(self.character_id, self.session_id, "Invalid summary", 5, 2)
    ...
    def test_get_session_summaries(self):
        ...
        summaries = self.memory.get_session_summaries(self.session_id, "anonymous")
    ...
    def test_get_character_summaries(self):
        ...
        summaries = self.memory.get_character_summaries(self.character_id, "anonymous")
    ...
    def test_get_character_summaries_with_limit(self):
        ...
        summaries = self.memory.get_character_summaries(self.character_id, "anonymous", limit=3)
        assert len(summaries) == 3

    def test_get_character_summaries_no_summaries(self):
        """Test retrieving summaries for character with no summaries."""
        summaries = self.memory.get_character_summaries("nonexistent_character", "anonymous")
        assert summaries == []
    ...
    def test_get_summaries_covering_offset(self):
        ...
        covering_summaries = self.memory.get_summaries_covering_offset(self.session_id, "anonymous", 4)
        ...
        covering_summaries = self.memory.get_summaries_covering_offset(self.session_id, "anonymous", 12)
        assert len(covering_summaries) == 1
        assert covering_summaries[0]["summary"] == "Summary 3"

        # Test offset not covered by any summary
        covering_summaries = self.memory.get_summaries_covering_offset(self.session_id, "anonymous", 9)
    ...
    def test_get_summaries_in_range(self):
        ...
        overlapping_summaries = self.memory.get_summaries_in_range(self.session_id, "anonymous", 2, 6)
        ...
        overlapping_summaries = self.memory.get_summaries_in_range(self.session_id, "anonymous", 16, 20)
        assert len(overlapping_summaries) == 0

        # Test range that partially overlaps
        overlapping_summaries = self.memory.get_summaries_in_range(self.session_id, "anonymous", 12, 20)
    ...
    def test_update_summary(self):
        ...
        # Add a summary
        summary_id = self.memory.add_summary(self.character_id, self.session_id, "Original summary", 0, 2)

        # Update the summary
        success = self.memory.update_summary(summary_id, "anonymous", "Updated summary text")
    ...
    def test_update_nonexistent_summary(self):
        ...
        success = self.memory.update_summary(999, "anonymous", "New text")
    ...
    def test_delete_summary(self):
        ...
        # Delete one summary
        success = self.memory.delete_summary(summary_id1, "anonymous")
        assert success is True
    ...
    def test_delete_nonexistent_summary(self):
        ...
        success = self.memory.delete_summary(999, "anonymous")
        assert success is False
    ...
    def test_delete_session_summaries(self):
        ...
        deleted_count = self.memory.delete_session_summaries(self.session_id, "anonymous")
        assert deleted_count == 2

        # Verify session1 summaries are gone
        session1_summaries = self.memory.get_session_summaries(self.session_id, "anonymous")
        assert len(session1_summaries) == 0

        # Verify session2 summaries remain
        session2_summaries = self.memory.get_session_summaries(session2, "anonymous")
    ...
    def test_clear_character_summaries(self):
        ...
        deleted_count = self.memory.clear_character_summaries(self.character_id, "anonymous")
        assert deleted_count == 1

        # Verify character1 summaries are gone
        char1_summaries = self.memory.get_character_summaries(self.character_id, "anonymous")
        assert len(char1_summaries) == 0

        # Verify character2 summaries remain
        char2_summaries = self.memory.get_character_summaries(character2, "anonymous")
    ...
    def test_get_max_processed_offset(self):
        ...
        max_offset = self.memory.get_max_processed_offset(self.session_id, "anonymous")
    ...
    def test_database_persistence(self):
        ...
        # Add a summary
        original_summary_id = self.memory.add_summary(self.character_id, self.session_id, "Persistent summary", 0, 3)
        ...
        summaries = new_memory.get_session_summaries(self.session_id, "anonymous")
        assert len(summaries) == 1
    ...
    def test_edge_case_single_message_summary(self):
        ...
        covering = self.memory.get_summaries_covering_offset(self.session_id, "anonymous", 5)
